...
function GroupOrChannelInfo() {
  ...
  return (
    <div className="relative h-[80%] w-[90%] bg-bg-primary pt-20">
      <button
        className="absolute right-5 top-8 rounded-lg bg-bg-secondary px-4 py-2 text-text-primary"
        onClick={toggleView}
      >
        {view === 'info' ? 'Edit' : 'Back'}
      </button>
      ...
      {view === 'edit' && openedChat.type === 'Group' ? (
        ...
        <>
          ...
          {openedChat.type === 'Channel' && (
            <div>
              <div
                className="absolute right-5 top-20 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full bg-bg-secondary text-3xl hover:bg-bg-button-hover"
