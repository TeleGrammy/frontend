...
const VoiceNotePlayer = ({ src, time, type }) => {
  ...
  return (
    <div className={`flex flex-col items-start rounded-3xl ${type === 'sent' ? 'bg-bg-message-sender' : 'bg-gray-700'} p-4 text-white shadow-md max-w-sm`}>
        ...
        <div className="flex-1">
          ...
          <div className="flex space-x-1 mb-1">
            {Array.from({ length: 15 }).map((_, index) => (
              <div
                key={index}
                className="h-3 w-1 bg-white rounded"
                style={{
      ...
      <div className="mt-2 flex justify-between w-full text-sm text-white/80">
        <span>{time}</span>
        <span>✔✔</span>
      </div>
    </div>
